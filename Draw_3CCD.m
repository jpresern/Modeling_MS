...
for l = 9%1:9        %   conditioning higher than 5.7 was not done as it is not possible to fit with boltzmann (not enough points)
...
testAmp = [conAmp, conAmp(2:end-l)+max(conAmp)];
...
offset = peakRecovery;
normal = offset./repmat(min(offset),[size(offset,1),1,1]);

...
% 1.One starts the function with entering "Modeling_DRG('DRG_TCM')" at the command window
%   workspace MUST be set to folder DRG_TCM
...
%   _v3:    includes the C(14) to model Hao2010 Fig2b. Fig2b was digitized.
%           The parameters that can control are:
%           1) PointWeights
%           2) StimulusTimeSpan
%           3) IndexOfStimuliToFit
%   _v4:    1) This version is adapted to normalized recording values in
%               3A, 3B & 6D. The file with experimental data is 
%               DRG_TCM_ExperimentalData_Hao2010_norm.mat
%           2) Variable A is fixes around 7 (constant-like) because the values are normalized.
%           3) Peaks of onset response curves are measured from 250th data
%               point on.
%   _v5:    1) Calls on the improved version of DRG_TCM_Engine
%   -v6:    1) Added fitting of Hao3G, H and I
%           2) Results of fits are brought into the function to avoid
%               calling _Report.m function
%           3) some fits are implemented in external functions
%           4) File loading/saving independent of the OS platform. Using
%               separate function (filename) to achieve that)
%           5) matlabpool close set after result output to save time


function Modeling_DRG_v6(Project_in)
%%
...
%%%% various types of global minimum search are written down here.
%%%% Currently implemented is "simulannealbnd" which requires options
...
%%% Following line outputs results, plots graphs and makes .pdf


% Modeling_DRG_Report(Project_in,horzcat('Results_',num2str(MaxInd+1)));
Modeling_DRG_Report_v4(Project,horzcat(Project,'_Results_',num2str(MaxInd+1)));
...
%%% Parameters
    ...
    % C10 penalize if min peak goes below -1.1
        ...
        if InitialFitParam.C_Weights(1) ~= 0 || InitialFitParam.C_Weights(2) ~= 0 || InitialFitParam.C_Weights(7) ~= 0 ||...
            ...
            peaksMeasured = -ExpData.Fig3A_Diagram.y'/100;      %%%POPRAVI V PRAVILNE VREDNOSTI V EXPDATA!!!!!!!!!!!!!!!!!!!!!!!!
